...
class Grid:
        ...
        if button == LEFT_CLICK:
            if self.game.player.active_equipped == BIONIC_GLASSES and not self.game.is_first_click:
                ...
                if self.grid[cell][CELL_STATE] == HIDDEN:
                        ...
                        if self.grid[cell][CELL_VALUE] == BOMB:
                            if self.game.player.active_equipped == MAGNIFIER:
                                ...
                                if jammer_near:
                                    ...
                                    self.game.player_is_dead = True

                                    current_time = pygame.time.get_ticks()
                                    delay = 3000
                                    if current_time + delay <= pygame.time.get_ticks():
                                        self.game.player_state = PLAYER_INACTIVE
                                        for item in self.game.player.discovered_items:
                                            item.amount = 0
                    ...
                    elif self.grid[cell][CELL_STATE] == NOT_HIDDEN and self.grid[cell][CELL_VALUE] < -1 and self.grid[cell][CELL_VALUE] != JAMMER and self.grid[cell][IS_COLLECTED] == NOT_COLLECTED:
    ...
    def is_circled(self, cell: tuple[int, int]):
        """
        Determines if the cell is circled by revealed or correctly flagged cells.
        ...
        -------
        bool
            True if the cell is circled; False otherwise.
        """
        all_cells = self.get_all_cells_around(cell)
        good_cells = 0
        for cell in all_cells:
