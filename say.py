...
class Speech_bubble:
   ...
   def __init__(self, game, speech: str):
      ...
      self.game = game
      self.speech = speech
      self.bubble_image = pygame.image.load(
          "sprites/bubble.png").convert_alpha()
   ...
   def display(self):
      ...
      self.bubble_image = pygame.transform.smoothscale(self.bubble_image,
                                                       (width, height))
      ...
      if current_time <= len(speech_list) * delay:
         ...
         if self.game.is_trading:
            bubble_position = (self.game.screen_width * 42 / 100 -
                               self.bubble_image.get_width(),
                               self.game.screen_height * 38 / 100 -
                               self.bubble_image.get_height())
            spike_position = (self.game.screen_width * 42 / 100 -
                              self.spike_image.get_width(),
                              self.game.screen_height * 38 / 100)
            ...
            for t in range(len(speech_list)):
               ...
               text_position = (self.game.screen_width * 42 / 100 -
                                self.bubble_image.get_width() * 95 / 100,
                                self.game.screen_height * 38 / 100 -
                                self.bubble_image.get_height() +
                                t * text_image.get_height() +
                                self.bubble_image.get_height() * 5 / 100)
               self.game.screen.blit(text_image, text_position)

         else:
            bubble_position = (self.game.player.x -
                               self.bubble_image.get_width() * 90 / 100,
                               self.game.player.y -
                               self.bubble_image.get_height() +
                               self.game.gui_width * 1 / 2)
            spike_position = (self.game.player.x +
                              self.bubble_image.get_width() * 10 / 100 -
                              self.spike_image.get_width(),
            ...
            for t in range(len(speech_list)):
               text_image = font.render(speech_list[t], True, (0, 0, 0))
               text_position = (self.game.player.x -
                                self.bubble_image.get_width() * 90 / 100 +
                                self.bubble_image.get_width() * 5 / 100,
                                self.game.player.y -
                                self.bubble_image.get_height() +
                                self.game.gui_width * 1 / 2 +
                                t * text_image.get_height() +
