...
class Trader:
    ...
    def __init__(self, game):
        ...
        self.trader_rect = pygame.Rect(self.game.screen_width * (239*100 / 600) / 100, self.game.screen_height * (78*100/360)/100, self.trader_image.get_width() * (91*100 / 600) / 100, self.trader_image.get_height() * (145*100 / 360) / 100)
    ...
    def handling_events(self):
        ...
        elif self.magnifier_rect.collidepoint(pygame.mouse.get_pos()) and self.game.items[COIN].amount >= 10:
            self.game.items[COIN].amount -= 10
            self.game.items[MAGNIFIER].picked()
            self.game.item_collected.play()

        elif self.shield_rect.collidepoint(pygame.mouse.get_pos()) and self.game.items[COIN].amount >= 15:
            self.game.items[COIN].amount -= 15
            self.game.items[SHIELD].picked()
            self.game.item_collected.play()

        elif self.upgrader_rect.collidepoint(pygame.mouse.get_pos()) and self.game.items[COIN].amount >= 20:
            self.game.items[COIN].amount -= 20
